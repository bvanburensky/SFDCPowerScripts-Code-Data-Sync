...
$onlyProcessFile = ""
$onlyProcessSObject = "Product2"
$onlyProcessDirectory = "sprint6"
#$onlyProcessDirectory = "sprint-5"

## Inputoutput subdir contain your deployment
$csvOutOverride = 'sprint6' 
...
$sReplaceFileName = "xRepInvoiceReplace.ps1"
...
$RenPath = $csvOutputPath + 'Account.invoice.csv'
$delPath = $csvOutputPath + 'MI_PEvent_Log__c.toUpsert.csv'
...
foreach ($letter in $AlphaBetU) {
  ...
  if ($rename) {
    ...
    Remove-Item $delPath
    write-host rename  $RenPath  -ForegroundColor yellow
    Rename-Item -Path $RenPath -NewName "MI_PEvent_Log__c.toUpsert.csv"
  }
  if ($deploy) {
