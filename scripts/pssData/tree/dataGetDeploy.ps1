...
$sourceOrgAlias = 'DIQA' 
$sourceOrgAlias = 'DI_Dev' 
$sourceOrgAlias = 'bvbtest19' 
...
$onlyProcessFile = "sObjs1Config.ps1"
$onlyProcessSObject = "Account"
$onlyProcessDirectory = "tree"
...
foreach ($letter in $AlphaBetU) {
  ...
  if ($retrieve) {
    ...
    $command = $scriptPath + "fxGetData.ps1 -sourceOrgAlias $sourceOrgAlias -csvOutputPath $csvOutputPath $commandParm $cmdAlpha  " 
...
# repull from target
$csvOutputPath = "scripts/data/org/" + $targetOrgAlias + "/"
if ($csvOutOverride -gt "") {
  $csvOutputPath = "scripts/data/org/" + $csvOutOverride + "/" + $targetOrgAlias + "/"
}
